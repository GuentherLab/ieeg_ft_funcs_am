...
if strcmp(dftreplace,'zero')
  ...
  %   diff_sig = diff(dat,1,2);
  % 
  %   m_sig = median(diff_sig,2,'omitnan');
  %   std_sig = std(diff_sig,1,2,'omitnan');
  %   n_std = 3;
  % 
  %   og_sig = dat;
  %   og_sig(diff_sig > n_std*std_sig + m_sig | diff_sig < -1*(n_std*std_sig + m_sig)) = 0;
  %   dat = og_sig;
  ...
  fdat = dat;
  fdat(isnan(fdat)) = 0; % remove nans 
  fdat = fft(fdat,[],2); % calculate fft
  idx = 1 + round(Fl/Fs*nsamples); % indexes of real frequencies
  idx = [idx; nsamples+2-idx(idx>1)]; % add indexes of imag frequencies

  fdat(:,idx) = 0;
  % filt = imag(ifft(fdat,[],2));
  filt = real(ifft(fdat,[],2));
  ...
  % ampl = {};
  % for i_f = 1:numel(Fl)
  %     tmp  = exp(1i*2*pi*Fl(i_f)*time);       % complex sin and cos 
  %     ampl{i_f} = 2*dat(:, sel)/tmp(:, sel);  % estimated amplitude of complex sin and cos on integer number of cycles
  % end
  % 
  % filt = dat;
  % for i_amp = 1:numel(ampl)
  %     tmp  = exp(1i*2*pi*Fl(i_amp)*time);
  %     est  = real(ampl{i_amp}*tmp);                   % estimated signal at this frequency
