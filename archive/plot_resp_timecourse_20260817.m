...
%
% inputs:
%       1. trials trial which must contain:
...
%           -times - must contain the syncing time in each trial.... only required if resp_aligned not provided
...
if ~strcmp(op.sort_cond,'')
    ...
    %% try to get rid of this conditional - instead, use the above condition and just set trials.sort_cond to 1 for all trials
