...
% fill empty trials w/ nans.... do not remove trials or else the trial indices will no longer align w/ other trial tables
empty_trials = cellfun(@isempty, trials.resp_unaligned);
med_length = ceil(median(cellfun(@length,trials.times(~empty_trials))));
for itrial = 1:height(trials)
    if isempty(trials.resp_unaligned{itrial})
        trials.times{itrial} = nan(1,med_length);
        trials.resp_unaligned{itrial} = nan(1,med_length);
    end
end
